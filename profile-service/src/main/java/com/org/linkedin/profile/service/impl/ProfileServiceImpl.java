...
public class ProfileServiceImpl implements ProfileService {
  ...
  public List<ProfileDTO> searchProfiles(
      ...
      String sortBy,
      String sortOrder) {
    return profileRepo.searchProfiles(query, city, state, company, headline, sortBy).stream()
        .map(profileMapper::toDto)
        .collect(Collectors.toList());
  }

  @Override
  public String updateCoverImage(UUID userId, org.springframework.web.multipart.MultipartFile image)
      throws IOException {
    TUserDTO tUserDTO = userService.getUserByKeyCloakId(userId).getBody().getResult();
