...
public class JobServiceImpl implements JobService {
    ...
    @Override
    public BasePageResponse<List<JobDTO>> searchJobs(String query, String title, String company, String location, String jobType, Pageable pageable) {
        Page<Job> jobPage = jobRepository.searchJobs(query, title, company, location, jobType, pageable);
        List<JobDTO> dtoList = jobMapper.toDto(jobPage.getContent());
    ...
    public List<JobApplicationDTO> getMyApplications(Authentication authentication) {
        UUID keycloakId = UUID.fromString(authentication.getName());
        UUID internalUserId = userService.getUserByKeyCloakId(keycloakId).getBody().getResult().getId();
        List<JobApplication> apps = jobApplicationRepository.findByApplicantIdOrderByCreatedAtDesc(internalUserId);
    ...
    public JobApplicationDTO updateApplicationStatus(UUID jobId, UUID applicantId, String status) {
        JobApplication application = jobApplicationRepository.findByJobIdAndApplicantId(jobId, applicantId)
